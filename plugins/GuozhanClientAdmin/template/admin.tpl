  ...
  <main class="gzca-main">
    ...
    {if $GZCA_TAB eq 'works'}
      ...
      <section class="gzca-panel gzca-works-panel">
        ...
        <div class="gzca-cover-help"><strong>封面显示位置</strong><span>设置的是作品所属的具体板块封面：所有板块都会显示在各自页面顶部；父级板块还会显示在前台分类入口卡片。当前封面会在作品行中明确标记，也可以单独下架封面，不会下架或删除作品。</span></div>
          ...
          <table class="gzca-table gzca-work-table">
            ...
            {foreach from=$GZCA_WORKS item=work}
              <tr>
                ...
                <td><div class="gzca-work-cell"><img src="{$work.thumb_url|escape:'html'}" alt=""><span><strong>{$work.display_name|escape:'html'}</strong><small>图片 ID {$work.id} · 原图 {$work.filesize_label|escape:'html'}</small>{if $work.is_category_cover}<em class="gzca-cover-badge" title="当前为 {$work.category_display_name|escape:'html'} 的封面">当前封面 · {$work.category_leaf_name|escape:'html'}</em>{/if}</span></div></td>
      ...
      <div class="gzca-category-layout {if $GZCA_EDIT_CATEGORY}is-editing{/if}">
        <section class="gzca-panel gzca-category-form-panel">
          ...
          <form class="gzca-stack-form" action="{if $GZCA_EDIT_CATEGORY}{$GZCA_URLS.categories|escape:'html'}&amp;edit={$GZCA_EDIT_CATEGORY.id}{else}{$GZCA_URLS.categories|escape:'html'}{/if}" method="post" data-category-form>
            ...
            <label class="gzca-field"><span>编号前缀</span><input type="text" name="code_prefix" value="{if $GZCA_EDIT_CATEGORY}{$GZCA_EDIT_CATEGORY.code_prefix|escape:'html'}{/if}" placeholder="例如 YH-FJ" maxlength="48" required><small>上传作品时会用它自动生成连续编号；普通保存只影响以后上传的作品，不会修改已有编号。</small></label>
