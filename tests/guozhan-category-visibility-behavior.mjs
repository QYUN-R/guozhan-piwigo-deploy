...
const instrumentedSource = source.replace(
  ...
  "  window.__GUOZHAN_TEST_HOOKS__.categories = { apiDirectoryGroups, apiDirectoryCardMarkup, apiCompactDirectoryCardMarkup, categoryContextFromList };\n  return;\n  renderStaticDirectory();"
);
...
const categoryRows = [
  { id: 29, id_uppercat: null, name: "中美协展览专项画稿", key: "caa-exhibitions", code_prefix: "ZX", rank: 10, kind: "exhibition", reserved: false, direct_upload: true, tn_url: "https://example.test/covers/caa.jpg" },
  ...
  { id: 30, id_uppercat: 29, name: "梅花之韵——2026·中国画花鸟作品展", key: "caa-plum-blossom", code_prefix: "ZX-MH", rank: 1, kind: "exhibition", reserved: false, direct_upload: true, tn_url: "https://example.test/covers/plum.jpg" },
...
assert.equal(groups[0].coverUrl, "https://example.test/covers/caa.jpg", "The directory group dropped its configured cover.");
assert.deepEqual(
...
assert.match(hooks.apiDirectoryCardMarkup(groups[0], 0), /directory-cover[^>]+caa\.jpg/, "Desktop category cards do not render their cover.");
assert.match(hooks.apiCompactDirectoryCardMarkup(groups[0], 0), /quick-category-cover[^>]+caa\.jpg/, "Home category cards do not render their cover.");
...
assert.equal(childContext.coverUrl, "https://example.test/covers/plum.jpg", "A child category did not retain its own cover.");
